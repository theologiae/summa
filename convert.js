 ...
 *    
 * NOTE: This scripts needs to be rewritten to use the XML provided by ccel, 
 * along with a data model of Summa in JS (class Question, class Article, class Directory) 
 * that would enable easier configuration & conversion to other formats
 * e.g. ePub, html
*/
...
let buildDir = (name, start, end, dirs=new Array(250)) => {
  ...
  if (dir.level >= 1) {
    dir.title = `${start+1}-${end}. ` + dir.title;
  }
